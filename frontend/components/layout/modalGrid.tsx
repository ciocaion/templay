...
function GridModal({ isOpen, onClose, onSelect }: NewModalProps) {
  ...
  const items = [
    <ButtonIcon icon={<GridViewOutlinedIcon />} text="Banner" onClick={() => onSelect('BANNER')} type={"BANNER"} />,
    <ButtonIcon icon={<GridViewOutlinedIcon />} text="Hero" onClick={() => onSelect('HERO')} type={"BANNER"} />,
    <ButtonIcon icon={<GridViewOutlinedIcon />} text="Card Standard" onClick={() => onSelect('PAGE')} type={"BANNER"} />,
  ];
